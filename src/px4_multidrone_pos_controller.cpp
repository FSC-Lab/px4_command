 ...
 *
 * Introduction:  PX4 controller for multi drone implimentation. UAV is labeled as UAV#. UAV# is added to the preflex of
 *very topic
 ...
 *         4. 通过command_to_mavros.h将计算出来的控制指令发送至飞控（通过mavros包）(mavros package will send the message
 *to PX4 as Mavlink msg)
...
bool CheckReferencePosition(const px4_command::ControlCommand& command_) {
  ...
  if (command_.Reference_State.position_ref[0] < geo_fence_x[0] ||
      command_.Reference_State.position_ref[0] > geo_fence_x[1] ||
      command_.Reference_State.position_ref[1] < geo_fence_y[0] ||
      command_.Reference_State.position_ref[1] > geo_fence_y[1] ||
      command_.Reference_State.position_ref[2] < geo_fence_z[0] ||
      command_.Reference_State.position_ref[2] > geo_fence_z[1]) {
    return false;
...
void PrintParam() {
  ...
  if (isMulti) {
    ...
    if (!isCorrectDrone) {
      ...
      cout << "WARNING!! The designated drone ID is : " << TargetdroneID
           << ". However, the current ID is : " << CurrentdroneID << endl;
    ...
    switch (Command_Now.Mode) {
      ...
      case command_to_mavros_multidrone::Payload_Stabilization: {
        ...
        if (emergencyflag) {
          ...
          throttle_sp[math_utils::Vector_Z] = _ControlOutput.Throttle[math_utils::Vector_Z];
          _AttitudeReference =
              px4_command_utils::ThrottleToAttitude(throttle_sp, 0.0);  // set the yaw angle command to zero
          _command_to_mavros.send_attitude_setpoint(_AttitudeReference);
        }
        break;
      }
      case command_to_mavros_multidrone::Payload_Land: {
        ...
        if (Command_Last.Mode != command_to_mavros_multidrone::Payload_Land) {
          ...
          Command_to_gs.Reference_State.position_ref[math_utils::Vector_Z] =
              0.7;  // set 0.7 for quadrotor hovering height
