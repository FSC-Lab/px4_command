...
void SubAuxiliaryState(const px4_command::AuxiliaryState::ConstPtr& msg, bool* IsReceived,
                       px4_command::AuxiliaryState& state) {
  *IsReceived = true;
  state = *msg;
}
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber subAuxiliaryState = nh.subscribe<px4_command::AuxiliaryState>(
      "/test/px4_command/auxiliarystate", 100,
      std::bind(&SubAuxiliaryState, arg::_1, &IsAuxiliaryReceived, FakeAuxstate));
  ros::Subscriber subFleetStatus = nh.subscribe<px4_command::FleetStatus>(
      "/test/px4_command/fleetstatus", 100,
      std::bind(&SubFleetStatus, arg::_1, &IsFleetStatusReceived, FaKeFleetStatus));
