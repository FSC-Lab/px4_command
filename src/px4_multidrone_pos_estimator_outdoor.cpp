...
void GetGPSPos(const nav_msgs::Odometry::ConstPtr& msg, px4_command::DroneState* dronestate) {
  ...
  dronestate->velocity[math_utils::Vector_Z] =
      -msg->twist.twist.linear.z;  // this is a potential bug in mavros node. (should be ENU)
}
...
bool ResponseToSetGPSHomeCall(px4_command::SetHome::Request& req, px4_command::SetHome::Response& res,
                              ros::Publisher* const pub, px4_command::HomePosition* const msg) {
...
void DisplayVisionResults(const Eigen::Vector3f& payloadposition, const Eigen::Vector3f& payloadvelocity,
  ...
  std::cout << "Payload inertial position X: " << payloadposition(math_utils::Vector_X)
            << " (m), Y: " << payloadposition(math_utils::Vector_Y)
            << " (m), Z: " << payloadposition(math_utils::Vector_Z) << " (m). \n";
  std::cout << "Payload inertial velocity X: " << payloadvelocity(math_utils::Vector_X) * 100.0
            << " (cm/s), Y: " << payloadvelocity(math_utils::Vector_Y) * 100.0
            << " (cm/s), Z: " << payloadvelocity(math_utils::Vector_Z) * 100.0 << " (cm/s). \n";
  std::cout << "The sample time is: " << sampletime << " (s) \n";
  std::cout << "Payload body position X: " << payloadpositionbody(math_utils::Vector_X)
            << " (m), Y: " << payloadpositionbody(math_utils::Vector_Y)
            << " (m), Z: " << payloadpositionbody(math_utils::Vector_Z) << " (m). \n";
  std::cout << "Payload body velocity X: " << payloadvelocitybody(math_utils::Vector_X) * 100.0
            << " (cm/s), Y: " << payloadvelocitybody(math_utils::Vector_Y) * 100.0
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber SubGPSGlobal = nh.subscribe<sensor_msgs::NavSatFix>(
      TopicMavrosGPSGlobal, 100, std::bind(&GetDroneLongitudeLatitude, arg::_1, &DroneStateData));
  // get the local position in ENU frame from mavros
  ros::Subscriber SubGPSPosition =
      nh.subscribe<nav_msgs::Odometry>(TopicMavrosGPSPos, 100, std::bind(&GetGPSPos, arg::_1, &DroneStateData));
  // get the battery status from mavros
  ros::Subscriber SubMavrosBattery = nh.subscribe<sensor_msgs::BatteryState>(
      TopicMavrosBattery, 100, std::bind(&GetUAVBattery, arg::_1, &DroneStateData));
  // get the connection state from mavros
  ros::Subscriber SubMavrosState =
      nh.subscribe<mavros_msgs::State>(TopicMavrosState, 100, std::bind(&GetMavrosState, arg::_1, &DroneStateData));
  // get the attitude and accelerations from IMU
  ros::Subscriber SubDroneAttitude =
      nh.subscribe<sensor_msgs::Imu>(TopicMavrosIMU, 100, std::bind(&GetDroneAttitude, arg::_1, &DroneStateData));
  // service call to set the gps home position
  ros::ServiceServer serverAction = nh.advertiseService<px4_command::SetHome::Request, px4_command::SetHome::Response>(
      TopicGCSGPSHome, std::bind(&ResponseToSetGPSHomeCall, arg::_1, arg::_2, &PubGPSHome, &GPSGlobalHome));
  // get the vision feedback from the camera

  // ros vision subscriber
  // Subvision = new ros::Subscriber Subvision;
  ros::Subscriber Subvision = nh.subscribe<geometry_msgs::PoseStamped>(
      "/pose_tag_body", 100, std::bind(&GetVisionFeedback, arg::_1, &PayloadPosition));
