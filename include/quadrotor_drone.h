...
void quadrotor_drone::loadparameter(const quadrotor_parameter& param) { parameter = param; }
...
px4_command::ControlOutput quadrotor_drone::outputdronecommand(const Eigen::Vector3d& accelCommand,
  ...
  command.throttleSetpoint = px4_command_utils::thrustToThrottleLinear(
      command.thrustSetpoint, parameter.liftmodel.motor_slope, parameter.liftmodel.motor_intercept);
...
std::string quadrotor_drone::getUAVname() { return parameter.uav_name; }
IMUdata quadrotor_drone::getProcessedIMU() { return IMU; }
drone_command quadrotor_drone::getDroneCommand() { return command; }
void quadrotor_drone::printf_param() {
  cout << parameter.uav_name << " parameter:  \n";
  cout << "Quad_MASS : " << parameter.Quad_MASS << " [kg] \n";
  cout << "Motor Curve Slop: " << parameter.liftmodel.motor_slope
       << " Motor Curve Intercept: " << parameter.liftmodel.motor_intercept << "\n";
...
void quadrotor_drone::printf_state() {
  ...
  cout << "Throttle Setpoint [X Y Z] : " << command.throttleSetpoint(math_utils::Vector_X) << " [] "
       << command.throttleSetpoint(math_utils::Vector_Y) << " [] " << command.throttleSetpoint(math_utils::Vector_Z)
       << " [] \n";
  cout << "Accbody: [X Y Z] : " << IMU.AccBody(math_utils::Vector_X) << " [m/s^2] " << IMU.AccBody(math_utils::Vector_Y)
       << " [m/s^2] " << IMU.AccBody(math_utils::Vector_Z) << " [m/s^2] \n";
  cout << "AccInertial: [X Y Z] : " << IMU.AccInertial(math_utils::Vector_X) << " [m/s^2] "
       << IMU.AccInertial(math_utils::Vector_Y) << " [m/s^2] " << IMU.AccInertial(math_utils::Vector_Z)
       << " [m/s^2] \n";
