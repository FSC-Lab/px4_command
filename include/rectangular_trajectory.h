...
Rectangular_Trajectory::Rectangular_Trajectory() { int state = 0; }
...
void Rectangular_Trajectory::LoadParameter(Rectangular_Trajectory_Parameter& parameter) {
  ...
  for (int i = 0; i < 4; i++) {
    theta[i] =
        atan2(path[i].P(math_utils::Vector_Y) - param_.center_y, path[i].P(math_utils::Vector_X) - param_.center_x);
  }
  target_theta[0] = 1;
  target_theta[1] = 2;
  target_theta[2] = 3;
