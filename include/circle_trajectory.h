...
void Circle_Trajectory::printf_result(px4_command::TrajectoryPoint& Circle_trajectory) {
  ...
  cout << "position  [X Y Z] : " << Circle_trajectory.position_ref[0] << " [m] " << Circle_trajectory.position_ref[1]
       << " [m] " << Circle_trajectory.position_ref[2] << " [m] " << endl;

  cout << "velocity [X Y Z] : " << Circle_trajectory.velocity_ref[0] << " [m/s] " << Circle_trajectory.velocity_ref[1]
       << " [m/s] " << Circle_trajectory.velocity_ref[2] << " [m/s] " << endl;

  cout << "acceleration [X Y Z] : " << Circle_trajectory.acceleration_ref[0] << " [m/s^2] "
       << Circle_trajectory.acceleration_ref[1] << " [m/s^2] " << Circle_trajectory.acceleration_ref[2] << " [m/s^2] "
       << endl;

  // cout << "jerk [X Y Z] : " << Circle_trajectory.jerk_ref[0] << " [m/s^3] "<< Circle_trajectory.jerk_ref[1]<<"
  // [m/s^3] "<<Circle_trajectory.jerk_ref[2]<<" [m/s^3] "<<endl;

  // cout << "snap [X Y Z] : " << Circle_trajectory.snap_ref[0] << " [m/s^4] "<< Circle_trajectory.snap_ref[1]<<"
  // [m/s^4] "<<Circle_trajectory.snap_ref[2]<<" [m/s^4] "<<endl;
}
