 ...
 *         4. ThrottleToAttitude ref to
 *https://github.com/PX4/Firmware/blob/master/src/modules/mc_pos_control/Utility/ControlMath.cpp
...
class pos_controller_cascade_PID {
 ...
 public:
  ...
  void _positionController(const px4_command::DroneState& _DroneState,
                           const px4_command::TrajectoryPoint& _Reference_State, Eigen::Vector3d& vel_setpoint);
...
void pos_controller_cascade_PID::_positionController(const px4_command::DroneState& _DroneState,
  ...
  {
    vel_setpoint[0] =
        _Reference_State.velocity_ref[0] + Kp_xy * (_Reference_State.position_ref[0] - _DroneState.position[0]);
    vel_setpoint[1] =
        _Reference_State.velocity_ref[1] + Kp_xy * (_Reference_State.position_ref[1] - _DroneState.position[1]);
  } else {
  ...
  {
    vel_setpoint[2] =
        _Reference_State.velocity_ref[2] + Kp_z * (_Reference_State.position_ref[2] - _DroneState.position[2]);
  } else {
...
void pos_controller_cascade_PID::cal_vel_error_deriv(const Eigen::Vector3d& error_now,
                                                     Eigen::Vector3d& vel_error_deriv) {
...
void pos_controller_cascade_PID::printf_result() {
  ...
  // cout << "Velocity_sp  [X Y Z] : " << vel_setpoint[0] << " [m/s] "<< vel_setpoint[1]<<" [m/s] "<<vel_setpoint[2]<<"
  // [m/s] "<<endl;

  // cout << "Vel_P_output [X Y Z] : " << vel_P_output[0] << " [m/s] "<< vel_P_output[1]<<" [m/s] "<<vel_P_output[2]<<"
  // [m/s] "<<endl;

  // cout << "Vel_I_output [X Y Z] : " << thurst_int[0] << " [m/s] "<< thurst_int[1]<<" [m/s] "<<thurst_int[2]<<" [m/s]
  // "<<endl;

  // cout << "Vel_D_output [X Y Z] : " << vel_D_output[0] << " [m/s] "<< vel_D_output[1]<<" [m/s] "<<vel_D_output[2]<<"
  // [m/s] "<<endl;

  cout << "thrust_sp    [X Y Z] : " << thrust_sp[0] << " [m/s^2] " << thrust_sp[1] << " [m/s^2] " << thrust_sp[2]
       << " [m/s^2] " << endl;
}
