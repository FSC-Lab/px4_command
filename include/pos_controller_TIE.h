...
class pos_controller_TIE {
 public:
  pos_controller_TIE(const std::string& drone_ID, ros::NodeHandle& main_handle) {
    ...
    serverAction = main_handle.advertiseService("/" + uav_pref + "/px4_command/action",
                                                &pos_controller_TIE::ResponseToActionCall, this);
    if (isPublishAuxiliarySate) {
      pubAuxiliaryState = main_handle.advertise<px4_command::AuxiliaryState_singleUAV>(
          "/" + uav_pref + "/px4_command/auxiliarystate_singleUAV", 1000);
    }
    // initialize
  }
  void printf_param();
 ...
 private:
  ...
  bool ResponseToActionCall(px4_command::SinglePayloadAction::Request& req,
                            px4_command::SinglePayloadAction::Response& res);
...
px4_command::ControlOutput pos_controller_TIE::pos_controller(const px4_command::DroneState& _DroneState,
  ...
  u_d = Lambda * (Payload_Mass * (Identity + Kpv) * B * (v_p + KL * r) + Quad_MASS * UAV_velocity + integral) /
        (TotalLiftingMass);
...
bool pos_controller_TIE::ResponseToActionCall(px4_command::SinglePayloadAction::Request& req,
                                              px4_command::SinglePayloadAction::Response& res) {
...
bool pos_controller_TIE::DroneGeoFenceCheck() {
  if (UAV_position(0) < geo_fence_x[0] || UAV_position(0) > geo_fence_x[1] || UAV_position(1) < geo_fence_y[0] ||
      UAV_position(1) > geo_fence_y[1] || UAV_position(2) < geo_fence_z[0] || UAV_position(2) > geo_fence_z[1]) {
    return false;
  } else {
    return true;
  }
...
void pos_controller_TIE::printf_result() {
  ...
  cout << "Pos Error [X Y Z] : " << pos_error(0) << " [m] " << pos_error(1) << " [m] " << pos_error(2) << " [m] "
       << endl;
  cout << "Vel Error [X Y Z] : " << vel_error(0) << " [m/s] " << vel_error(1) << " [m/s] " << vel_error(2) << " [m/s] "
       << endl;
  ...
  cout << B << endl;
  cout << "W_hat [X Y Z] : " << W_hat(math_utils::Vector_X) << " [N], " << W_hat(math_utils::Vector_Y) << " [N], "
       << W_hat(math_utils::Vector_Z) << " [N]. \n";
  ...
  cout << "payload velocity from mocap is [X Y Z] : " << payload_velocity_mocap(math_utils::Vector_X) << " [m/s] "
       << payload_velocity_mocap(math_utils::Vector_Y) << " [m/s] " << payload_velocity_mocap(math_utils::Vector_Z)
       << " [m/s] " << endl;

  cout << "payload position from mocap is [X Y Z] : " << payload_position_mocap(math_utils::Vector_X) << " [m] "
       << payload_position_mocap(math_utils::Vector_Y) << " [m] " << payload_position_mocap(math_utils::Vector_Z)
       << " [m] " << endl;
...
void pos_controller_TIE::printf_param() {
  cout << ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Payload control method in TIE paper (Parameter)  <<<<<<<<<<<<<<<<<<<<<<<<<<<"
       << endl;
