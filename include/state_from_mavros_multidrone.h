...
class state_from_mavros_multidrone {
 public:
  ...
  state_from_mavros_multidrone(const std::string &ID)
    ...
    // 【订阅】无人机当前位置 坐标系:ENU系
    // （此处注意，所有状态量在飞控中均为NED系，但在ros中mavros将其转换为ENU系处理。所以，在ROS中，所有和mavros交互的量都为ENU系）
    //  本话题来自飞控(通过Mavros功能包 /plugins/local_position.cpp读取), 对应Mavlink消息为LOCAL_POSITION_NED (#32),
    //  对应的飞控中的uORB消息为vehicle_local_position.msg
    position_sub = state_nh.subscribe<geometry_msgs::PoseStamped>(mavros_local_position_pose, 10,
                                                                  &state_from_mavros_multidrone::pos_cb, this);

    // 【订阅】无人机当前速度 坐标系:ENU系
    //  本话题来自飞控(通过Mavros功能包 /plugins/local_position.cpp读取), 对应Mavlink消息为LOCAL_POSITION_NED (#32),
    //  对应的飞控中的uORB消息为vehicle_local_position.msg
    velocity_sub = state_nh.subscribe<geometry_msgs::TwistStamped>(mavros_local_position_velocity_local, 10,
                                                                   &state_from_mavros_multidrone::vel_cb, this);

    // 【订阅】无人机当前欧拉角 坐标系:ENU系
    //  本话题来自飞控(通过Mavros功能包 /plugins/imu.cpp读取), 对应Mavlink消息为ATTITUDE (#30),
    //  对应的飞控中的uORB消息为vehicle_attitude.msg
    attitude_sub =
