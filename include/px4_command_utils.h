...
void printf_command_control(const px4_command::ControlCommand& _ControlCommand) {
  ...
  switch (_ControlCommand.Mode) {
    ...
    case command_to_mavros::Takeoff:
      ...
      cout << "Position_Ref [X Y Z] : " << _ControlCommand.Reference_State.position_ref[0] << " [ m ] "
           << _ControlCommand.Reference_State.position_ref[1] << " [ m ] "
           << _ControlCommand.Reference_State.position_ref[2] << " [ m ] " << endl;
...
void prinft_drone_state(const px4_command::DroneState& _Drone_state) {
  ...
  cout << "Position [X Y Z] : " << _Drone_state.position[0] << " [ m ] " << _Drone_state.position[1] << " [ m ] "
       << _Drone_state.position[2] << " [ m ] " << endl;
  cout << "Velocity [X Y Z] : " << _Drone_state.velocity[0] << " [m/s] " << _Drone_state.velocity[1] << " [m/s] "
       << _Drone_state.velocity[2] << " [m/s] " << endl;
  cout << "Attitude [R P Y] : " << _Drone_state.attitude[0] * 180 / M_PI << " [deg] "
       << _Drone_state.attitude[1] * 180 / M_PI << " [deg] " << _Drone_state.attitude[2] * 180 / M_PI << " [deg] "
       << endl;
  cout << "Att_rate [R P Y] : " << _Drone_state.attitude_rate[0] * 180 / M_PI << " [deg/s] "
       << _Drone_state.attitude_rate[1] * 180 / M_PI << " [deg/s] " << _Drone_state.attitude_rate[2] * 180 / M_PI
       << " [deg/s] " << endl;
}
...
void cal_pos_error(const px4_command::DroneState& _DroneState, const px4_command::TrajectoryPoint& _Reference_State,
  ...
  // 对于速度追踪子模式，则无位置反馈
  if (_Reference_State.Sub_mode == command_to_mavros::XY_VEL_Z_POS ||
      _Reference_State.Sub_mode == command_to_mavros::XY_VEL_Z_VEL) {
    for (int i = 0; i < 2; i++) {
      pos_error[i] = 0;
    }
  }

  if (_Reference_State.Sub_mode == command_to_mavros::XY_POS_Z_VEL ||
      _Reference_State.Sub_mode == command_to_mavros::XY_VEL_Z_VEL) {
    pos_error[3] = 0;
  }
}
...
Eigen::Vector3d thrustToThrottle(const Eigen::Vector3d& thrust_sp) {
  ...
  for (int i = 0; i < 3; i++) {
    throttle_sp[i] = MOTOR_P1 * pow(thrust_sp[i], 4) + MOTOR_P2 * pow(thrust_sp[i], 3) +
                     MOTOR_P3 * pow(thrust_sp[i], 2) + MOTOR_P4 * thrust_sp[i] + MOTOR_P5;
